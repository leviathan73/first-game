...
const explosionAudio = new Audio("./assets/explode.wav");
...
export class Ship extends Body2d {
  ...
  constructor() {
    ...
    const geometry = new THREE.CircleGeometry(0.1, 32, -0.5 + Math.PI, 1);
...
export class Meteor extends Body2d {
  ...
  size = 3;
  /** @returns {Array<Body2d>} */
  explode(shootVector) {
    const fragments = [];
    if (this.size > 1) {
      let helperVector = shootVector.rotate(
        Math.PI + Math.PI / (4 - Math.floor(Math.random() * this.size * 2))
      ); //+ Math.PI / this.size
      for (let i = this.size; i > 0; i--) {
        const fragment = new Meteor(this.size - 1);
        fragment.setRadius(this.size * 10);
        fragment.setPosition2(this.p.add(helperVector));
        helperVector.setMagnitude2(this.baseSpeed - this.size);
  ...
  draw(context) {
    ...
    context.moveTo(this.shapeSteps[0].x, this.shapeSteps[0].y);
